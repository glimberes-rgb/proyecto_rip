...
## Arquitectura e Integración del Modelo
...
Datos de Entrada: predicciones.csv + limites_parroquiales.geojson
     - Uso previsto: Ingesta de la capa espacial y métricas estadísticas.

Backend (Flask): api_routes.py (Controlador API REST)
     - Uso previsto: Carga el CSV indexado en un diccionario en memoria (O(1)) || Normaliza los códigos de parroquia a 6 dígitos.

Output de API: JSON Enriquecido
     - Uso previsto: Une la geometría con el riesgo predicho y sus probabilidades.

Frontend (Web): Leaflet.js + JavaScript Asíncrono
     - Uso previsto: Consumo asíncrono (fetch) y renderizado dinámico de coropletas.
...
└── webapp/                 # Módulo principal de la aplicación de Flask
    ...
    │
    ├── data/               # Almacenamiento de archivos planos 
    ...
    └── static/             # Assets Estáticos del Servidor
        ├
        └── js/
            └── map.js      # Controlador de mapas (L.map, Leyendas, Hover y Popups)
